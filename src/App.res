...
let make = () => {
  ...
  let onResize = () => setSize(_ => {width: window["innerWidth"], height: window["innerHeight"]})
  ...
  let onCompile = _ => worker["postMessage"]({"type": "Compile", "source": prompt})

  let onStep = _ => worker["postMessage"]({"type": "Step"})

  let onContinue = _ => worker["postMessage"]({"type": "Continue"})

  <main id="app">
    <Canvas vm size />
    <div id="overlay">
      <div id="overlay-left">
        <Terminal prompt setPrompt vm onCompile onStep onContinue /> <Examples setPrompt />
      </div>
      <Machine vm />
    </div>
  </main>
