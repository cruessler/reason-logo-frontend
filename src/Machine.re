...
let renderVariables = (variables: Vm.variables) => {
  <table>
    <tbody>
      {Js.Dict.entries(variables) |> Array.map(renderBinding) |> React.array}
    </tbody>
  </table>;
};
...
let renderLocalScope = (address, variables) => {
  <li>
    <h3> {React.string("Local@" ++ string_of_int(address))} </h3>
...
let renderStack = stack => {
  <div id="stack">
    ...
    <ul>
      {Array.map(entry => <li> {React.string(entry)} </li>, stack)
       |> React.array}
    </ul>
  </div>;
};
...
let renderScopes = scopes => {
  <div id="scopes">
    <h2> {React.string("Scopes")} </h2>
    <ul> {Array.map(renderScope, scopes) |> React.array} </ul>
  </div>;
};

let renderInstructions = (instructions, programCounter) => {
  let currentInstruction =
    Belt.Array.get(instructions, programCounter)
